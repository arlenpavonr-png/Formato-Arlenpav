 ...
 * 4. Deploy → Web app → Execute as: Me → Anyone → copia URL /exec
 * 5. Gumroad → Settings → Advanced → Ping → pega la URL
 *
 * COLUMNAS Sheet (fila 1): CODIGO | PLAN | CLIENTE | EMAIL | VENCIMIENTO | ACTIVO
 */
...
// ─── GET: validación licencia (PWA JSONP) ────────────────────────────────────
...
function doGet(e) {
  ...
  let result = { valido: false, mensaje: 'Código de licencia inválido.' };

  if (!codigo) {
    result.mensaje = 'Código requerido.';
    return respondJsonp_(result, callback);
  }
  ...
  const colVenc = headers.indexOf('VENCIMIENTO');

  if (colCodigo < 0) {
    result.mensaje = 'Error de configuración del Sheet.';
    return respondJsonp_(result, callback);
  }

  for (let i = 1; i < data.length; i++) {
    const rowCodigo = String(data[i][colCodigo] || '').trim().toUpperCase();
    if (rowCodigo !== codigo) continue;

    const activo = colActivo >= 0
      ? String(data[i][colActivo] || '').trim().toUpperCase()
      : 'SI';

    if (activo !== 'SI' && activo !== 'SÍ') {
      result.mensaje = 'Licencia inactiva.';
      return respondJsonp_(result, callback);
    }

    if (colVenc >= 0) {
      const venc = data[i][colVenc];
      if (venc instanceof Date && !isNaN(venc.getTime())) {
        if (stripTime_(venc) < stripTime_(new Date())) {
          result.mensaje = 'Licencia vencida.';
          return respondJsonp_(result, callback);
        }
      }
    }

    result = { valido: true, mensaje: 'Licencia válida.' };
    return respondJsonp_(result, callback);
  }
...
function resolvePlanFromPayload_(payload) {
  ...
  // Respaldo: producto gratis por precio 0
